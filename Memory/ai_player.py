...
class AIPlayer(Player):
    ...
    def play_random_cards(cards_ids):
        ...
        second_card = choice(
            [id_card for id_card in cards_ids if id_card != first_card]
        )
    ...
    def search_pairs(self):
        return next((list_ids['position']
                     for image, list_ids in self.memory_board.items()
                     if len(list_ids['position']) == 2), None)

    def search_matching_card(self, card_id, image):
        try:
            return next(id_card
                        for id_card in self.memory_board[image]['position']
                        if id_card != card_id)
        except KeyError:
            return None
